...
*
*          Copyright (C) 2007-2024 by T.TAKASU, All rights reserved.
...
*         May, 2020
*     [6] ublox-AG, UBX-23003447-R01, u-blox F10 TIM 3.01 Interface Description,
*         March, 2023
*     [7] ublox-AG, UBX-22008968-R01, u-blox F9 HPG 1.32 u-blox F9 high precision
*         GNSS receiver Interface Description, May, 2022
*
...
*           2023/08/04 1.30 support NEO-F10T [6]
*           2024/02/24 1.31 support ZED-F9P-15B (L5 and NavIC) [7]
*-----------------------------------------------------------------------------*/
#include "rtklib.h"
...
/* UBX GNSSId to system (ref [2] 25, [6] 1.5.2) ------------------------------*/
...
/* UBX SigId to signal (ref [5] 1.5.4, [6] 1.5.4) ----------------------------*/
...
/* decode UBX-RXM-RAWX: multi-GNSS raw measurement data (ref [3][4][5][6]) ---*/
...
static int decode_inav(raw_t *raw, int sat, int off)
{
    ...
    if (raw->len<56) {
        trace(2,"ubx inav length error: len=%d\n",raw->len);
        return -1;
    }
    for (i=0;i<10;i++,p+=4) {
        setbitu(buff,32*i,32,U4(p));
...
static int decode_rxmsfrbx(raw_t *raw)
{
    ...
    switch (sys) {
        ...
        case SYS_IRN: return decode_inav(raw,sat,8);
...
static int decode_rxmqzssl6(raw_t *raw)
{
    static int count[16][2]={{0}};
    uint8_t *p=raw->buff+6;
    double cn0,time;
    char buff[1024]="",*q=buff;
    int i,ver,prn,msg,nerr,stat;
...
static int decode_ubx(raw_t *raw)
{
    ...
    switch (type) {
        ...
        case ID_RXMQZSSL6: return decode_rxmqzssl6(raw);
