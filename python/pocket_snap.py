...
def search_sig(data, sig, sys, prn, dif, fs, fi, rrate):
    ...
    if cn0 >= THRES_CN0:
        ...
        #print('%s : SIG=%-5s C/N0=%5.1f dB-Hz DOP=%9.3f m/s COFF=%12.9f ms' %
...
def pos_dop(data, spos):
    ...
    for i in range(10):
        ...
        x += dx
        #print('(%d) N=%2d  POS=%s  RES=%10.3f' % (i, n, pos_str(x), norm(v)))
        if norm(dx) < 1.0:
            return x[:3]
    return np.zeros(3)
...
def pos_coff(time, data, rr, nav):
    ...
    for i in range(10):
        ...
        dx, res, rank, s = np.linalg.lstsq(H[:n,:], v[:n], rcond=None)
        x += dx
        #print('(%d) N=%2d  POS=%s  CLK=%9.6f %9.6f  RES=%10.3f' % (i, n,
        #    pos_str(x), x[3] / CLIGHT, x[4], np.sqrt(np.dot(v, v) / n)))
        if norm(dx) < 1e-3:
