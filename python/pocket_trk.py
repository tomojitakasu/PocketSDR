...
def print_head():
    print('%9s %5s %4s %6s %9s %5s %-13s %9s %9s %10s %5s %4s %4s' %
        ('TIME(s)', 'SIG', 'PRN', 'STATE', 'LOCK(s)', 'C/N0', '(dB-Hz)',
        'COFF(ms)', 'DOP(Hz)', 'ADR(cyc)', 'SYNC', '#NAV', '#ERR'))
...
def init_plot(sig, prn, ch, file):
    ...
    ax1, p1 = plot_corr_env (fig, rect1, pos)
...
# show usage -------------------------------------------------------------------
def show_usage():
    print('Usage: pocket_trk.py [-sig sig] [-prn prn[,...]] [-plot prn] [-toff toff]')
    print('           [-f freq] [-fi freq] [-ti tint] [-ts tspan] [-log file] file')
    exit()
...
#   Synopsis
# 
#     pocket_trk.py [-sig sig] [-prn prn[,...]] [-plot prn] [-toff toff]
#         [-f freq] [-fi freq] [-ti tint] [-ts tspan] [-log file] file
# 
#   Description
# 
...
if __name__ == '__main__':
    sig, prns, plot = 'L1CA', [1], False
    fs, fi, toff, tint, tspan = 12e6, 0.0, 0.0, 0.05, 1.0
    ch = {}
    file, log_file = '', ''
    log_lvl = 4
    ...
    print_head()
    if plot:
        fig, ax, p = init_plot(sig, prns[0], ch[0], file)
    ...
    ncol = 0
    tt = time.time()
    log(3, '$LOG,%.3f,%s,%d,START FILE=%s FS=%.3f FI=%.3f TOFF=%.3f' %
        (0.0, '', 0, file, fs * 1e-6, fi * 1e-6, toff))
