...
#define BUFF_SIZE   (1<<16)      // buffer size (bytes)
    ...
    double freq, double bw, double gain)
{
    ...
    const size_t ch = 0;
    size_t chs[1] = {ch};
printf("sdr_sdev_open driver=%s fmt=%d rate=%.0f freq=%.0f bw=%.0f gain=%.0f\n", driver, fmt, rate, freq, bw, gain);
...
static void *reader_thread(void *arg)
{
    ...
    if (ovcnt > 0) {
        fprintf(stderr, "SoapySDRDevice_readStream overflow cnt=%d\n",
            ovcnt);
    }
    return NULL;
}
