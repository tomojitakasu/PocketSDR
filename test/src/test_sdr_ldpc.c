...
// flip bit positions ----------------------------------------------------------
static void flip_positions(uint8_t *bits, const int *pos, int npos)
{
...
static void test_sdr_decode_ldpc_cnv2_sf3_no_error(void)
{
    ...
    TEST_ASSERT_EQ_INT(548, hex_to_bits(hex, syms, sizeof(syms)));
    memset(dec, 0, sizeof(dec));
    
    TEST_ASSERT_EQ_INT(0, sdr_decode_LDPC("CNV2_SF3", syms, 548, dec));
    assert_bits_inv_prefix(dec, syms, 274);
}
...
static void test_sdr_decode_ldpc_bcnv2_no_error(void)
{
    ...
    TEST_ASSERT_EQ_INT(576, hex_to_bits(hex, syms, sizeof(syms)));
    memset(dec, 0, sizeof(dec));
    
    TEST_ASSERT_EQ_INT(0, sdr_decode_LDPC("BCNV2", syms, 576, dec));
    assert_bits_eq(dec, syms, 288);
}
